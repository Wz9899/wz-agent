...
## 背景：Agent = 模型 + 上下文 + 工具 + 循环

用 Cursor 写代码，看它搜索代码库、改多个文件、跑测试直到通过；用一个助手自动订票、
整理报告——这些产品的共同点是：它们不再"你问一句答一句"，而是**自主规划步骤、
调用工具完成任务、根据结果调整策略**。这样的系统就叫 Agent（智能体）。

业界把它拆成一个简洁的公式：

> **Agent = LLM（大脑）+ 上下文（眼睛）+ 工具（手脚）**

- **LLM 是大脑**：理解意图、思考规划、做出判断。编码能力是它在海量人类代码上
  训练出来的，不是外部代码赋予的；
- **上下文是眼睛**：每个决策点收到的信息——环境观察、用户要求、任务进展；
- **工具是手脚**：读写文件、执行命令、向用户提问——模型借工具感知世界并行动。

但光有这三样还跑不起来。模型每轮只能"想一步"，需要有人把它包进一个循环里：
把上下文发给它 → 执行它决定要调的工具 → 把观察喂回去 → 再来一轮，
直到它认为任务完成。这个运行与治理层就是 **Harness（装具）**，
写出来是一段伪代码：
...
repeat:
    context = 稳定前缀(系统提示 + 工具定义) + trajectory
    decision = Model(context)
    trajectory.append(decision)

    if decision 没有 tool_call:
        return decision.answer          # 任务完成
    for call in decision.tool_calls:
        observation = Harness.execute(call)   # 校验后执行
        trajectory.append(observation)
```

业内也把这个关系写成 **Agent = Model + Harness**：Model 只负责决策，
Harness 负责"模型之外、进程之内"的一切——构造上下文、暴露工具接口、维护状态、
约束行为边界。同一家厂商的同一个模型，配不同的 harness 就是完全不同的产品。

**wz-agent 就是一个用 Python 从零实现的 Harness**（约 3.3k 行）：
它接上任意 OpenAI 兼容的模型 API（DeepSeek / GLM / …），让模型能真正在你的
项目目录里读写文件、跑验证、管任务。下面按模块讲这辆车是怎么造的。
...
**Workflow 与自主 Agent 的分界。** 编排模式里"工作流"指执行路径由代码预先写死
（LLM 只在每个节点内部干活），"自主 Agent"指路径由模型实时决定。Anthropic 总结的
构建原则是：能简单就不复杂——先考虑单次 LLM 调用能不能解决，再考虑固定工作流，
只有真正需要多步动态规划和执行路径时才上自主 Agent，因为自主性通常意味着更高的
延迟与成本。**wz-agent 的架构正是这条原则的体现**：交互会话里模型在 ReAct 循环中
自主路由（因为编码任务的路径无法预知），而 triage / to-tickets 这类路径固定的
流程则写成 headless 工作流——两条腿各走各的路。

**行业正在从"提示工程"走向"Harness 工程"。** 发展脉络是：Prompt Engineering
（打磨提示词）→ Context Engineering（系统化管理模型可见的信息）→ **Harness
Engineering**（组织模型之外的执行环境：约束、验证、纠正）→ Loop / Graph
Engineering（多人多阶段的协作编排）。对强模型而言，harness 质量的影响力已经超过
提示词技巧——Terminal Bench 2.0 上同一批前端 Top 模型换个 harness 得分可以从
52.8% 提到 66.5%。这就是 wz-agent 这类项目的存在意义：**harness 的每一行代码，
都是在补足或放大模型的能力。**

回到定位总结：**wz-agent 在 pi 与 Claude Code 之间、偏向 pi**——与 pi 共享
"相信模型、harness 只做减法"的信念，差异在于三点：① 用 Python 写就，代码量比 pi
小一个数量级，每一行都适合学习与魔改；② 有成型的**任务票流水线**（拆解→分诊→
按票实现），这是 pi 刻意不做的编排层；③ 不设插件系统，扩展就是改源码。适用场景：
想在中文环境下理解 harness 每一行的原理、并按自己需求改造的开发者。
